...
import { Pool } from "pg";
import { loadLatestInventoryProviderSnapshot, persistInventoryProviderSnapshot } from "../src/lib/inventory-provider-cache-store";
...
try {
  ...
  const committed = await persistInventoryProviderSnapshot({ pool, snapshot });
  assert.equal(committed.status, "COMMITTED");
  assert.equal(committed.recordCount, 1);

  const deduplicated = await persistInventoryProviderSnapshot({ pool, snapshot });
  assert.equal(deduplicated.status, "DEDUPLICATED");

  await assert.rejects(
    () => persistInventoryProviderSnapshot({ pool, snapshot: { ...snapshot, sourceHash: "conflicting-source-hash" } }),
    /INVENTORY_SNAPSHOT_IDENTITY_COLLISION/,
  );

  const loaded = await loadLatestInventoryProviderSnapshot({
    pool,
    providerId: snapshot.providerId,
    dealershipId: snapshot.dealershipId,
  });
  assert.ok(loaded);
  assert.equal(loaded.sourceHash, snapshot.sourceHash);
  assert.equal(loaded.records[0]?.modelYear, "2026.5");
  assert.equal(loaded.records[0]?.vin, "1N4BL4DV9SN320880");
