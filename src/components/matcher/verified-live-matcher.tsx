...
function LiveVehicleCard({ vehicle, onPass, onShortlist }: { vehicle: Vehicle; onPass: () => void; onShortlist: () => void }) {
  return <article className="overflow-hidden rounded-[24px] border border-white/15 bg-slate-950 shadow-2xl">
    <div className="relative aspect-[16/10] overflow-hidden bg-[#0a0f16]">
      <img src={vehicle.image} alt={`${vehicle.year} ${vehicle.make} ${vehicle.model} ${vehicle.trim}`} className="h-full w-full object-cover" loading="eager" />
      <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-slate-950 to-transparent" />
      <div className="absolute left-4 top-4 rounded-full border border-emerald-300/30 bg-black/70 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.14em] text-emerald-200 backdrop-blur">Verified live</div>
    </div>
    <div className="p-5 sm:p-6">
      <p className="text-xs font-bold text-emerald-300">VIN {vehicle.id}</p>
      <div className="mt-2 flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-3xl font-black text-white">{vehicle.year} {vehicle.make} {vehicle.model}</h3><p className="mt-1 text-sm text-slate-400">{vehicle.trim} · {vehicle.type} · {vehicle.drivetrain}</p></div><p className="text-2xl font-black text-amber-300">{formatMoney(vehicle.price)}</p></div>
      <div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-slate-300"><span className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1.5">{vehicle.mileage.toLocaleString()} mi</span>{vehicle.exteriorColor && <span className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1.5">{vehicle.exteriorColor}</span>}{vehicle.cityMpg && vehicle.highwayMpg && <span className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1.5">{vehicle.cityMpg}/{vehicle.highwayMpg} MPG</span>}</div>
      <div className="mt-6 grid grid-cols-2 gap-3"><button onClick={onPass} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-rose-400/25 bg-rose-400/10 font-bold text-rose-200"><X size={18} /> Pass</button><button onClick={onShortlist} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-emerald-400/25 bg-emerald-400/10 font-bold text-emerald-200"><Check size={18} /> Garage</button></div>
    </div>
  </article>;
}

function MiniCatalogCard({ vehicle }: { vehicle: Vehicle }) {
  return <Link href={`/vehicles/${vehicle.id}`} className="min-w-[255px] snap-start overflow-hidden rounded-2xl border border-white/10 bg-[#0d131b] hover:border-amber-300/30"><div className="aspect-[16/10] overflow-hidden bg-black"><img src={vehicle.image} alt={`${vehicle.year} ${vehicle.make} ${vehicle.model}`} className="h-full w-full object-cover" loading="lazy" /></div><div className="p-4"><p className="text-[9px] font-black uppercase tracking-[.18em] text-emerald-300">Verified live</p><h4 className="mt-2 font-black text-white">{vehicle.year} {vehicle.make} {vehicle.model}</h4><p className="mt-1 text-xs text-slate-500">{vehicle.trim} · {vehicle.drivetrain}</p><p className="mt-3 font-black text-amber-300">{formatMoney(vehicle.price)}</p><p className="mt-3 text-[11px] font-bold text-amber-300">Open vehicle →</p></div></Link>;
}
