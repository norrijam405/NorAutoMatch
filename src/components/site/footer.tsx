  ...
  return (
    <footer className="border-t border-white/10 bg-slate-950/80">
      <div className="shell grid gap-10 py-14 md:grid-cols-[1.2fr_.8fr_1fr]">
        <div>
          <Logo />
          <p className="mt-5 max-w-md text-sm leading-6 text-slate-400">A direct, payment-first shopping experience from a salesperson at Orr Nissan West. One person, one number, no lead-queue runaround.</p>
      ...
      <div className="border-t border-white/10 bg-white/[.02]">
        <div className="shell flex flex-col gap-4 py-6 text-sm sm:flex-row sm:items-center sm:justify-between">
          <div><p className="font-black text-white">NorAuto Match operates through {brand.dealer.name}</p><p className="mt-1 text-xs text-slate-500">{brand.dealer.address}</p></div>
      ...
      <div className="border-t border-white/5">
        <div className="shell flex flex-col gap-3 py-6 text-xs leading-5 text-slate-500 sm:flex-row sm:items-start sm:justify-between">
          <p>© {new Date().getFullYear()} NorAuto Match. Personal sales brand at Orr Nissan West.</p>
          <p className="max-w-3xl sm:text-right">Vehicle availability and pricing are subject to change. Payment examples are estimates only, exclude taxes, title, license, and other disclosed charges, and are not an offer of credit. All vehicle sales, financing, paperwork, and delivery are completed by Orr Nissan West or another approved licensed selling dealership.</p>
        </div>
      </div>
    </footer>
  );
